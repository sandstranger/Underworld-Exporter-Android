...
public class event_processor : UWClass
{
    ...
    public event_processor()
    {
        ...
        if (!DataLoader.ReadStreamFile(GameModel.CurrentModel.BasePath + GameWorldController.instance.SCD_Ark_File_Selected, out scd_ark_file_data))
        {
            Debug.Log(GameModel.CurrentModel.BasePath + GameWorldController.instance.SCD_Ark_File_Selected + " File not loaded");
            return;
        }
